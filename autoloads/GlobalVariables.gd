...
func mk_operator(scene, int_frame, pos, num_scale = 1):
...
func mk_number(scene, number, decs, pos, num_scale = 1, x_sep = 20, comma_sep = 20, comma_sep2 = 5, comma_y = 5):
	...
	for digit in number:
		...
		num_list.append(dig)
	if decs != null:
		comma = GlobalVariables.mk_operator(scene, 5, pos + Vector2(cnt*x_sep - comma_sep, comma_y), num_scale)
